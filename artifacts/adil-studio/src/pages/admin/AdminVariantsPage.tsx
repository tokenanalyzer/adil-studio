...
type Variant = { id: string; slug: string | null; name: string | null; created_at: string | null; };
...
export default function AdminVariantsPage() {
  ...
  async function load() {
    const { data, error } = await supabase.from("variants").select("id, slug, name, created_at").order("created_at", { ascending: true });
    if (error) { setError(error.message); } else { setItems((data ?? []) as Variant[]); }
    setLoading(false);
  }
  ...
  async function handleCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("variants").insert({ name: String(fd.get("name") || "").trim(), slug: String(fd.get("slug") || "").trim() });
  ...
  async function handleUpdate(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const { error } = await supabase.from("variants").update({ name: String(fd.get("name") || "").trim(), slug: String(fd.get("slug") || "").trim() }).eq("id", String(fd.get("id")));
    if (error) { alert(error.message); return; }
    load();
  }
  ...
  return (
    <main className="container" style={{ padding: "64px 0", display: "grid", gap: 24 }}>
      <section style={{ display: "grid", gap: 8 }}>
        ...
        <p style={{ color: "var(--muted)", maxWidth: 720 }}>Manage variant records using only safe existing fields.</p>
      </section>
      ...
      <section style={{ border: "1px solid var(--border)", borderRadius: 20, padding: 16, display: "grid", gap: 14 }}>
        ...
        <form onSubmit={handleCreate} style={{ display: "grid", gap: 12 }}>
          <label style={labelStyle}><span>Name</span><input name="name" required placeholder="Premium" style={fieldStyle} /></label>
          <label style={labelStyle}><span>Slug</span><input name="slug" required placeholder="premium" style={fieldStyle} /></label>
          <button type="submit" style={primaryBtn}>Create variant</button>
        </form>
      </section>

      <section style={{ display: "grid", gap: 12 }}>
        <h2 style={{ fontSize: 24 }}>Existing variants</h2>
        {items.length === 0 ? <div style={{ border: "1px solid var(--border)", borderRadius: 18, padding: 16 }}>No variants found.</div> : (
